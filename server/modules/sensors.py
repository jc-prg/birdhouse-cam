...
class BirdhouseSensor_org(threading.Thread):
    ...
    def raise_error(self, connect, message):
        """
        raise error
        """
        if connect:
    ...
    def connect(self):
        ...
        if "rpi_active" in self.config.param["server"] and self.config.param["server"]["rpi_active"]:
            try:
                ...
                self.raise_error(connect=True, message="Could not load " + self.param["type"] +
                                                       " sensor module: " + str(e))
                return
            ...
            try:
                ...
                self.raise_error(connect=False, message="Initial load " + self.param["type"] + " not OK: " + str(e))
                return
            ...
            self.raise_error(connect=True, message="No sensor available: requires Raspberry Pi / activate" +
