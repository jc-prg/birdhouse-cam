    ...
    def measure_status(self, init=False):
        ...
        if init:
            ...
            if self.statistics and self.measure_last + self.measure_time > time.time():
                ...
                if self.get_processing_performance("api_GET") != -1 and "server " in self.get_processing_performance("api_GET"):
                    self.statistics.set("srv_api_status", self.get_processing_performance("api_GET")["server"])
