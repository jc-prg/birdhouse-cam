...
class myViews_v2(threading.Thread):
    ...
    def createIndex(self, server):
        ...
        if self.adminAllowed():  content["links"]  = self.printLinksJSON(link_list=("favorit","today","backup","cam_info"))
        else:                    content["links"]  = self.printLinksJSON(link_list=("favorit","today","backup"))
    ...
    def createList(self, server):
        ...
        if date_backup != "":
           ...
           if self.adminAllowed(): content["links"] = self.printLinksJSON(link_list=("live","favorit","today_complete","videos","backup"), cam=which_cam)
           else:                   content["links"] = self.printLinksJSON(link_list=("live","favorit","videos","backup"), cam=which_cam)
    ...
    def createBackupList(self, server):
        ...
        if self.adminAllowed(): content["links"] = self.printLinksJSON(link_list=("live","favorit","today","today_complete","videos"), cam=which_cam)
        else:                   content["links"] = self.printLinksJSON(link_list=("live","favorit","today","videos"), cam=which_cam)
    ...
    def createCompleteListToday(self, server):
        ...
        content["subtitle"]    = myPages["today_complete"][0] + " (" + self.camera[which_cam].name +", " + str(count) + " Bilder)"
        content["links"]       = self.printLinksJSON(link_list=("live","favorit","today","videos","backup"), cam=which_cam)
    ...
    def createVideoList(self, server):
        ...
        if self.config.exists("videos"):
           ...
           if len(files_delete) > 0 and self.adminAllowed(): content["entries_delete"] = files_delete


        content["view_count"] = ["all","star","detect","recycle"]
        content["subtitle"]   = myPages["videos"][0] # + " (" + self.camera[which_cam].name +", " + str(len(files_all)) + " Videos)"
        
        if self.adminAllowed():  content["links"]  = self.printLinksJSON(link_list=("live","favorit","cam_info","today","backup"))
        else:                    content["links"]  = self.printLinksJSON(link_list=("live","favorit","today","backup"))
