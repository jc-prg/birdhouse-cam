...
class myViews_v2(threading.Thread):
    ...
    def createList(self, server):
        ...
        if date_backup != "":
            ...
            if self.admin_allowed():
                content["links"] = self.printLinksJSON(link_list=("live", "favorit", "today_complete", "videos", "backup"), cam=which_cam)
            else:
        ...
        if files_all != {}:
            ...
            if self.admin_allowed(): header = True
            else:                   header = False
    ...
    def createBackupList(self, server):
        ...
        if self.admin_allowed(): content["links"] = self.printLinksJSON(link_list=("live", "favorit", "today", "today_complete", "videos"), cam=which_cam)
    ...
    def createVideoList(self, server):
        ...
        if self.admin_allowed():  content["links"]  = self.printLinksJSON(link_list=("live", "favorit", "cam_info", "today", "backup"))
