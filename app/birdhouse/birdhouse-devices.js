...
function birdhouseDevices(title, data, show="settings", subset="all") {
	var html        = "";
	var index       = [];
	var status_obj  = data["STATUS"]["object_detection"];
...
function birdhouseDevices_cameras(data, subset="") {
    ...
    if (subset != "short") {
	    for (let camera in cameras) {
            ...
            if (app_data["STATUS"]["object_detection"]["active"]) {
                var camera_object = camera.toUpperCase() + ": Object detection";
                index_info[camera_object] = {};
                index_info[camera_object]["active"] = cameras[camera]["object_detection"]["active"];
                index_info[camera_object]["group"]  = "";
                index_info[camera_object]["id"]     = camera + "_object";
                index_info[camera_object]["type"]   = "object";
                index_info[camera_object]["status"] = ["active", "error"];
                }
            ...
            html_temp += tab.start();
            html_temp += tab.row("Active:",     birdhouse_edit_field(id="set_active_"+camera, field="devices:cameras:"+camera+":active", type="toggle", options="false:true", data_type="boolean"));
            ...
            html_entry += tab.row("&nbsp;",                     birdhouseDevices_cameras_resolutions(camera, cameras[camera]["source"] ));
            ...
            if (settings["server"]["detection_active"]) {
                ...
                var loading_info  = app_data["STATUS"]["object_detection"]["status_details"];
                html_entry = tab.start();
                html_entry += tab.row("- Object Detection:",     birdhouse_edit_field(id="set_detect_active_"+camera, field="devices:cameras:"+camera+":object_detection:active", type="select", options="true,false", data_type="boolean"));
                ...
                html_entry += tab.row("- Detection Threshold:",  birdhouse_edit_field(id="set_detect_threshold_"+camera, field="devices:cameras:"+camera+":object_detection:threshold", type="input", options="", data_type="float") + " %");
                html_entry += tab.row("- Detection Similarity:", birdhouse_edit_field(id="set_detect_similarity_"+camera, field="devices:cameras:"+camera+":object_detection:similarity", type="select", options="true,false", data_type="boolean") + " %");
                ...
                html_entry += tab.row("- Detection Model:",      birdhouse_edit_field(id="set_detect_models_"+camera, field="devices:cameras:"+camera+":object_detection:model", type="select", options=model_options, data_type="string"));
...
function birdhouseDevices_cameras_resolutions(camera, source="") {
    ...
    var resolutions_available = "";

    if (app_data["STATUS"]["devices"]["available"]["video_devices_complete"][source] && app_data["STATUS"]["devices"]["available"]["video_devices_complete"][source]["resolutions"] != undefined) {
        resolutions_available = app_data["STATUS"]["devices"]["available"]["video_devices_complete"][source]["resolutions"];
        }
    ...
    if (source == "")                                   { source = getValueById("set_source_"+camera); }

    var resolution_on_change = "setValueById(\"set_resolution_"+camera+"\", this.value);";
    var resolution_options = {};
    resolution_options[resolution_act] = resolution_act + "   [current]";
    resolution_options[resolution_max] = resolution_max + "   [max]";

    Object.keys(resolutions_available).forEach(key => {
        var res_array = resolutions_available[key];
        for (var i=0;i<res_array.length;i++) {
            if (res_array[i] != resolution_max && res_array[i] != resolution_act && !resolution_options[res_array[i]]) {
                resolution_options[res_array[i]] = res_array[i];
                }
            }
        });

    var current_available_resolutions = "<u>" + source + "</u>";
    if (resolutions_available != "") {
        current_available_resolutions += ": " + JSON.stringify(resolutions_available);
    ...
    html  = "<div id='resolution_per_device_"+camera+"'>";
    if (current_available_resolutions.indexOf("N/A") < 0) {
        var select = birdhouse_edit_field(id="select_source_"+camera, field="", type="select_dict_sort", options=resolution_options, data_type="string", on_change=resolution_on_change);
        html += "<i>available: </i>";
        html += select;
        }
    else {
    ...
    setTextById("resolution_per_device_"+camera, html);
    return html;
    }
...
function birdhouseDevices_sensors(data) {
	...
	for (let sensor in sensors) {
        ...
        html_entry += tab.start();
		html_entry += tab.row("Active:", birdhouse_edit_field(id="set_active_"+sensor, field="devices:sensors:"+sensor+":active", type="toggle", options="false:true", data_type="boolean"));
...
function birdhouseDevices_relays(data) {
	...
	for (let relay in relays) {
        ...
        html_entry += "<div class='camera_info_text'>";
        html_entry += tab.start();
		html_entry += tab.row("Active:", birdhouse_edit_field(id="set_active_"+relay, field="devices:relays:"+relay+":active", type="toggle", options="false:true", data_type="boolean"));
...
function birdhouseDevices_weather(data) {
    ...
    html_entry += tab.start();
    html_entry += tab.row("Active:", birdhouse_edit_field(id="set_weather_active", field="weather:active", type="toggle", options="false:true", data_type="boolean"));
...
function birdhouseDevices_microphones(data) {
	...
	for (let micro in micros) {
        ...
        html_entry += tab.start();
		html_entry += tab.row("Active:",      birdhouse_edit_field(id="set_active_"+micro, field="devices:microphones:"+micro+":active", type="toggle", options="false:true", data_type="boolean"));
...
function birdhouseDevices_cameraSettings (data) {
	...
	for (let camera in camera_settings) {
        ...
        if (camera_properties[camera]["properties_new"] && Object.keys(camera_properties[camera]["properties_new"]).length > 0) {
            ...
            this_camera_bw   = camera_settings[camera]["image"]["black_white"];
            this_camera_properties["Black_White"] = [this_camera_bw, "rw", "boolean", [false, true, null]];
        ...
        for (var i=0;i<camera_settings_write.length;i++) {
            ...
            if (this_camera_type == "new") {
                ...
                if (value == "black_white") {
                    ...
                    data_edit += birdhouse_edit_save(id="edit_bw_"+camera, "set_black_white_"+camera, camera);
                    }
                else if (data_type == "float" || data_type == "integer") {
                    data_edit  = birdhouse_edit_field(id="set_"+value+"_"+camera, field="devices:cameras:"+camera+":image_presets:"+key, type="range", options=range, data_type=data_type);
