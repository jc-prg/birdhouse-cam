//--------------------------------------
// jc://birdhouse/object_detection_view/
//--------------------------------------
...
class BirdhouseObjects {
    ...
    */
    create (data) {
        ...
        let html = "";
        let title = lang("BIRDS_DETECTED");
        let detections = data["DATA"]["data"]["entries"];
        let server_status = app_data["STATUS"]["server"];

        if (!detections && server_status["view_archive_loading"] !== "done") {
            let progress_info = "<i><div id='loading_status_object'></div></i>";
            appMsg.alert(lang("DATA_LOADING_TRY_AGAIN") + "<br/>" + progress_info);
            return false;
        }

        this.tab.style_cells = {
            "vertical-align": "top",
            "padding": "3px"
        }

        // list of all available detections
        let all_labels = "<div id='label_all' class='detection_label_function' onclick="+this.name+"'.open();"+this.name+".highlight_labels(\"all\",\"label_key_list\");'>&nbsp;&nbsp;"+lang("ALL_LABELS")+"&nbsp;&nbsp;</div>";
        let all_labels_list = Object.keys(detections);
        all_labels_list.sort();

        for (let i=0;i<all_labels_list.length;i++) {
            let key = all_labels_list[i];
            let onclick = "bhObjects.open(label=\""+key+"\");"+this.name+".highlight_labels(\""+key+"\",\"label_key_list\");";
            all_labels += "<div id='label_"+key+"' class='detection_label' onclick='"+onclick+"'>&nbsp;&nbsp;" + this.bird_lang(key) + "&nbsp;&nbsp;</div>";
        }
        ...
        for (let i=0;i<all_labels_list.length;i++) {
            ...
            let key            = all_labels_list[i];
            let value          = detections[key];
            value["type"]      = "label";

            let default_dates     = "";
            let favorite_label    = "";
            let entry_information = "";

            if (value["detections"]["favorite"] > 0 ) {
                //let onclick = "birdhousePrint_load(\"FAVORITES\",\""+app_active.cam+"\", \"all-dates\", \""+key+"\");";
                let onclick = "birdhousePrint_page(page=\"FAVORITES\",cam=\""+app_active.cam+"\", date=\"all-dates\", label=\""+key+"\");";
            ...
            Object.entries(value["detections"]["default_dates"]).forEach(([camera, date_list])=>{
                ...
                for (let k=0;k<date_list.length;k++) {
                    let stamp      = date_list[k];
                    let date       = stamp.substring(6,8) + "." + stamp.substring(4,6) + "." + stamp.substring(2,4);
                    //let onclick    = "birdhousePrint_load(view=\"TODAY\", camera=\""+camera+"\", date=\""+stamp+"\", label=\""+key+"\");";
                    let onclick    = "birdhousePrint_page(page=\"TODAY\", cam=\""+camera+"\", date=\""+stamp+"\", label=\""+key+"\");";
            ...
            let image_count = (value["detections"]["favorite"] + value["detections"]["default"]);
            ...
            let image      = "<div class='detection_title_image'>" + birdhouse_Image(this.bird_lang(key), key, value) + "</div>";
            image          = image.replace("<img", "<img style='max-height:200px;width:auto;'")
            html_entry    += this.tab.row(image, entry_information);
            html_entry    += this.tab.end();

            let bird_key = this.bird_lang(key);
    ...
    */
    open(label="all", default_groups=false, active_page) {

        if (!default_groups) {
            let all_labels = document.getElementById("label_key_list").innerHTML.split(",");
            ...
            let groups = getTextById("group_list").split(" ");
            for (let i=0;i<groups.length;i++) {
                if (groups[i] !== "") {
                    let labels = getTextById("group_labels_"+active_page+"_"+groups[i]);
    ...
    */
    highlight_labels(key, list="") {
        ...
        if (list !== "" && document.getElementById(list)) {
            let label_keys = document.getElementById(list).innerHTML.split(",");
            if (key !== "all") { label_keys.push("all"); }
            for (let i=0;i<label_keys.length;i++) {
                let this_key = label_keys[i];
    ...
    */
    bird_lang(bird_class) {
        let key = bird_class.toUpperCase();
    ...
    */
    labels_edit(date, time, camera, label="") {

        let api_request = [];
        let message     = "<b>"+lang("EDIT_OBJECT_LABELS")+"</b><hr/>";
        let cmd         = this.name+".label_edit_request();";
        let date_time;
        ...
        birdhouse_apiRequest("GET", api_request, "", this.labels_edit_load, "", this.name+"label_edit");

        appMsg.dialog(message, cmd, 80, 85);
    }
    ...
    */
    labels_edit_load(data) {

        let message     = "";
        let id          = getTextById("edit_label_time");
        let entry       = data["DATA"]["data"]["entries"][id];
        let detections  = entry["detections"];
        let imageURL    = entry["directory"] + entry["hires"];
        ...
        message += "<tr style='font-weight:bold;'><td style='width:15px'>#</td><td style='width:55px'>" + lang("IMAGE") + "</td><td>" + lang("CURRENT_LABEL") + "</td><td>" + lang("EDIT_LABEL") + "</td></tr>";
        ...
        for (let i=0;i<detections.length;i++) {
            ...
            let image       = birdhouse_ImageCropped("labels_edit_img_"+count, imageURL, coordinates, "max-height:80px;max-width:80px;border:1px solid black;");
            img_ids        += "labels_edit_img_"+count+",";

            let select      = "<select id='labels_edit_img_"+count+"'>";
            ...
            Object.keys(app_bird_names).forEach(key => {
                select     += "<option value='" + key.toLowerCase() + "'>" + this.bird_lang(key) + "</option>";
            });
            select         += "<option style='color: red; font-weight: bold;' value='DELETE'>"+lang("DELETE").toUpperCase().replace("&OUML","&Ouml")+"!</option>";
            select         += "</select>";

            message        += "<tr><td style='width:10px'>" + count + "</td><td style='width:50px'>" + image + "</td><td>" + label + label_org + "</td><td>"+select+"</td></tr>";
            count          += 1;
        }
        message += "</table>";
        message += "<br/>&nbsp;<br/>";
    ...
    */
    label_edit_request() {
        let entry_id   = getTextById('edit_label_time');
        let entry_date = getTextById('edit_label_date');
        let label_ids  = getTextById('edit_label_id_list').split(",");
        let label_data = "";

        for (let i=0;i<label_ids.length-1;i++) {
            let value = getValueById(label_ids[i]);
            label_data += i + "::" + value + ",";
        }

        let api_request = ["edit-labels", entry_date, entry_id, label_data];
        birdhouse_apiRequest("POST", api_request, "", birdhouse_AnswerOther, "", this.name+".label_edit_request");
